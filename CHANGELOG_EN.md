...
**`@tags` metatags and the module header reach search: a tag outweighs prose, a description is no longer truncated together with its tags, and "what is this module about" shows up in the object passport.**

> Context. The 1C code-markup rules put a header on every module and a description with `@tags`/`@depends` metatags on every exported procedure (the `code-docs` grammar). The index did not see them: a comment was truncated to 240 characters FROM THE START, while the grammar puts tags at the end — on the 1C dump only 12 of 691,228 term rows contained the substring `@tags`. A query equal to a procedure's own tags (`весы настройки`) matched 19,238 rows and the reference procedure was not even in the top 50: the trigram query «весы» does not match «весов» in prose. The module header was not stored at all. Spec: `docs/tz-module-header-and-tags.md`.

### Added

- **Metatags as a separate, heavier FTS column.** `@tags` and `@depends` lines are parsed apart from prose: `@tags` values (comma-separated) go into the new `procedure_enrichment.tags` column and EACH as its own phrase at the front of the terms — so that trigrams match the whole word («весы») instead of a slice of a phrase («…службе весов из…»). The column is a second FTS column ranked as `bm25(fts, 1.0, 4.0)`: a tag is written by hand, a stronger signal than a word that happens to occur in a description. The 240-character limit now applies to prose only, so tags at the end of a long description are no longer lost. `@layer`, `@security`, `@todo` and `@example` are deliberately not indexed — they add more noise than signal — but they no longer leak into prose either.
- **The module header — table `module_enrichment(path, header, tags, depends)`.** The comment at the top of a `.bsl` before the first declaration: the block counts as a header if it is followed by a blank line, `#Область`, `#Если`, `Перем` or the end of file; a block that runs into `Процедура`/`Функция`/`&Аннотация` with no blank line in between is the first procedure's description, not a header. Header tags additionally go into the terms of EVERY procedure of that module, which is why no separate module search was needed. It is maintained on the same beat as terms: the full pass, the parse-phase collector and the per-file incremental path (editing a header updates the row, deleting the file removes it).
- **Columns `comment_head` and `comment_len` on procedures.** `comment_head` is the first line of the description (up to 160 characters) — `search_terms` shows it instead of the whole terms string. `comment_len` is the description length before truncation; `0` means "no description above the declaration", and `bsl_sql` counts the "exported procedures without a contract" audit over that column (on the 1C dump: 1,133 exported procedures without a description out of 1,523 in the project's own common modules — the starting point for markup).
- **Tool output.** `search_terms` returns `tags`, `comment_head` and `module_tags` (the module header's tags, by the path inside `proc_key`) next to the terms; empty fields are omitted so they do not read as "no data". `get_object_profile` shows the first line of the header (`header`) and its tags for every module, and the merged tag list in the folded module map. The `bsl_sql` table list gained `module_enrichment` and the new `procedure_enrichment` columns.

### Changed

- **The mechanical terms signature is now `mech:v2`.** The incremental path rewrites terms only for changed files, so after a binary upgrade a database would keep the old layer until the next full reindex. Now, when the extension opens the database and sees rows with an outdated signature, it rebuilds ONLY the terms layer — from the module text already stored in the database (`file_contents`), without reindexing metadata or graphs. While the pass runs the folder counts as indexing and `health` answers "not ready", exactly as during indexing. Rows of LLM enrichment (a signature other than `mech:%`) are left alone. If a module's text is not in the database (oversize), the fallback is an ordinary full reindex.
- **The terms FTS index migrates to two columns** (`terms`, `tags`) with a `rebuild` from the content table and recreated triggers — an FTS5 table's column set is fixed at creation, and `CREATE TRIGGER IF NOT EXISTS` does not rewrite an existing trigger. The table and the triggers are checked independently, so a migration interrupted halfway is healed by the next database open. `search_terms` now MATCHes the table (both columns); an index from an earlier build, which has no such column yet, is served by the previous query instead of failing.

### Verification

- **Unit and integration tests:** `cargo test --workspace --all-targets` — 851 passed, 0 failed (was 838). New ones: tags beyond the 240-character boundary, `@` lines kept out of prose, `@security` producing no terms, a description made of tags only, phrase order and dedup with module tags, five module-header parsing cases, the FTS migration to two columns with rebuild and triggers, the full pass and the incremental path (editing a header updates the row, deleting the file removes terms and header), collector → staging → tables, the signature-change terms pass, and tag weighting in `search_terms` output.
- **Live check on the 1C dump** (26,000 `.bsl`, 691,228 term rows). The FTS migration to two columns with a full index rebuild — 26.9 s; the signature-change terms pass — 691,228 procedures in 44.6 s (a full reindex of the same folder takes 394 s); 4 modules had no stored text in the database, which is reported as a warning. 4,606 module headers were collected (31 with metatags), 74 of the 1,062 own modules with code. A full reindex of the same configuration through the other path (the parse-phase collector) yields 4,607 headers and the same 691,228 procedures, 47 with tags and 109,966 with a description — the one-header difference falls exactly on the modules whose text was not in the database. For `КРБ_ИнтеграцияВесов/Ext/Module.bsl::ПрочитатьНастройки`: `tags = 'весы, настройки'`, `comment_len = 705`, `comment_head` — the first line of the description. `search_terms(query="весы настройки")` puts that procedure first (before the change it was outside the top 50 of 19,238), `search_terms(query="весы")` fills the top 10 with the weighing modules thanks to header tags, and `get_object_profile` shows the module header and its tags.
- **The tag weight was chosen by measurement** over ten "where is X implemented" questions from real 1C tasks (weighing integration, ship unloading, tallyman, gradations): the sum of the reference procedure's positions is 28 at weight 1.0, **17 at 4.0** and 14 at 8.0. The difference between 4 and 8 comes from a single question (position 6 vs 3), both inside the top 10, so weight 4 was kept: as code markup spreads there will be far more tags, and "a tag outweighs everything" costs more than it gains. Control questions whose reference has no tags return an identical top 20 at all three weights — no degradation there by construction: an empty column contributes nothing to BM25.
...
- **Working files of neighbouring processes woke the daemon for nothing.** The file watcher filtered events only by excluded directories and name patterns; it never looked at the file category. An application log, a queue database and its journal in a neighbouring directory change every couple of seconds — and on every such event the daemon opened a transaction, updated extras, truncated the WAL and wrote three lines. Zero work done: such files never enter the index. Measured on the working installation: **3,051 batches out of 3,093 were empty and 77% of the log lines were their trace**, pushing everything useful out through rotation. An event is now dropped when the file demonstrably is not indexed. Directories and extensionless paths pass as before: a directory event expands its contents, and a deleted directory can no longer be told from a file.
...
> Context. A continuation of the full review of the data-collection layer. Same method: one configuration exported two ways, with per-table number comparison. That comparison is exactly what exposed the two Designer-format defects — they are invisible while you look at a single format.

### Fixed

- **EDT exports had no role rights, no configuration-level links and no module registry.** Parsing of that format stopped right after the shared pass over object descriptions, skipping three phases entirely. Role rights live in a separate file whose contents match the Designer format — only the file name and directory depth differ, so the parser is shared. Configuration-level links (subsystem and exchange-plan contents, defined-type targets, functional-option location and contents) live inside the very object descriptions already being read, so no extra disk walk was needed. The module registry got its own phase for the EDT layout: identifiers of forms and commands come from the owning object's description, because a form file carries no identifier of its own. On a standard accounting configuration (Designer format in brackets): rights 0 → 48,476 (48,489), modules 0 → 18,263 (18,291), configuration-level edges 0 → 41,708 (41,543). The impact map of a catalogue matches exactly: 262 = 262 structural references, 29 rights from 5 roles in both formats. The object version stays empty for EDT on purpose — that format has no dump manifest, and a surrogate value must not be invented: the debugger checks it.
- **Every nested subsystem was lost in EDT exports.** The walk went exactly two directory levels deep, while a nested subsystem sits deeper. The walk is now driven by a queue: a subsystem's subdirectories are appended as it goes, and appended before any checks — otherwise a whole branch would be lost when a subsystem's own description fails to read. On a standard accounting configuration: 73 → 617 subsystems, against 621 in the Designer format.
- **Common form handlers reached the index in neither format.** Form collection looked for the "form inside an object" layout, but a common form has no forms directory at all — it is an object in its own right. The owner key is now the export folder name under which its module already sits, and the form name is its own name. The handler tool needed no change: it already normalises singular to plural. On a standard accounting configuration 0 → 428 forms in the Designer format and 0 → 427 in EDT; 344 on a standard trade configuration.
- **A command module named after its own object lost its owner.** The owner lookup walked up the path until a directory name matched the object name — and for such a command the command's own directory came first, not the object's. The owner was then looked for next to it, never found, and the module was skipped silently. The losses were exactly the most typical case: a report or data-processor command named after the object itself — 91 modules out of 587 on a standard accounting configuration. The walk now stops at the commands directory, whose parent is the object. Found by format comparison: the EDT registry produced 587 command modules against 496, with an identical file count on disk.
- **Incremental updates never worked for EDT exports.** Two independent causes, either sufficient on its own. First, description files were not listed as text and counted as binary — there was no search over that format's metadata at all, and the file watcher raised no events for them. Second, batch classification knew only the Designer format: it required the `xml` extension, an object directly inside a type folder and specific file names, so no branch ever matched. The incremental path now has its own branch: editing an object rebuilds just its registry row, structure, data links and event subscription; forms, role rights and the module registry are updated too. The file tree plays the role of the dump manifest — an object counts as deleted when its description disappears, and its links, forms and modules go with it.

### Added

- **Search over 1C:EDT export metadata.** Object descriptions, forms and role rights of that format are now indexed as text — `grep_text`, `search_text` and `read_file` work over them, as they long have for the Designer format. Large files (configuration description, rights, form) are exempt from the size limit, like their same-named counterparts.

### Testing

- **Unit tests:** `cargo test --workspace` — 704 passed, 0 failed (was 696). New ones cover: nested EDT subsystems; EDT role rights; four kinds of configuration-level edges; the EDT module registry with an empty object version; a common form in both formats; a command named after its object; incremental updates of an object, a form and rights; object removal on a vanished description.
- **Live smoke** on one configuration in two export formats, both force-reindexed. Every effect confirmed by numbers; the formats agree.
- **Incremental-update rig** (a copy of two catalogues and a role under a separate alias): a synonym edit in an object description reached the index within seconds — previously nothing changed over 45 seconds of watching; deleting the description removed the object along with its modules and links.
- **Federation smoke** on three node databases: common forms and command modules appeared, and the handler tool answers for a common form.
