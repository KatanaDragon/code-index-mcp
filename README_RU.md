...
Полное руководство. Краткая витрина — [README.md](README.md), английская версия — [README_EN.md](README_EN.md).

**Rust-native индекс кода для AI-агентов. Статический бинарник. Production-grade поддержка BSL/1С.**

Один статический бинарник под Windows/Linux/macOS — без рантайма, без зависимостей. Индексирует крупные репозитории за секунды, отдаёт результат AI-агенту через MCP за миллисекунды. 32 инструмента: 20 универсальных + 12 для конфигураций 1С:Предприятие.

## Что внутри

- **Производительность.** Бухгалтерия на 88 тыс. файлов индексируется целиком за 5 мин 51 с, запуск на готовом индексе — 5,3 с, ответ на запрос — около 10 мс. Подходит для монорепо 100K+ файлов.
- **32 инструмента MCP.** 20 универсальных (функции, классы, вызывающие и вызываемые, содержимое файлов, поиск по коду) + 12 для 1С (структура и паспорт объекта, обработчики форм, подписки на события, граф вызовов, граф связей данных, регистраторы движений, права ролей, карта влияния, произвольный SQL только на чтение).
- **Native BSL/1С.** Парсит выгрузки конфигураций 1С:Предприятие 8.3 как из Конфигуратора (XML), так и из 1С:EDT (`.mdo`). Граф связей данных (рёбра объект→объект по ссылочным типам реквизитов) для типовой бухгалтерии — 65 421 ребро.
...
| `grep_code` | **(v0.8.0)** Regex-поиск по содержимому **code-файлов** (`.py`, `.bsl`, `.rs`, `.ts` и др.) через таблицу `file_contents` (zstd-decode в Rust). Параметры аналогичны `grep_text`: `regex`, `path_glob?`, `language?`, `limit?`, `context_lines?`. С v0.66.0 оба инструмента принимают также `pattern` — буквальную подстроку без учёта регистра (экранируется перед поиском), с тем же смыслом, что у `grep_body`. Дополняет `grep_body` (тот ищет только в телах функций/классов). Oversize-файлы пропускаются. **Параметр `scope`** (`code` / `text` / `all`) у обоих инструментов: объектные XML выгрузки 1С (`Catalogs/X.xml`, документы, регистры, подсистемы) хранятся как код с `language=xml_1c`, а `Configuration.xml`, `Form.xml`, `Rights.xml` — как текст, поэтому вопрос «где в выгрузке встречается имя» раньше требовал двух вызовов с разными фильтрами; `scope="all"` ищет по обеим категориям одним вызовом (код первым, текст на остаток общего `limit`), в ответе `by_category` и `truncated_by_category`; `language` при этом сужает только код. Без `scope` поведение прежнее. Удалённые репо на сборке без `scope` обслуживаются двумя форвардами со слиянием на стороне вызывающего |
...
При наличии BSL-репо в `daemon.toml` (`language = "bsl"`) автоматически добавляются 12 BSL-инструментов:
...
| `find_path_bsl` | Цепочка вызовов между двумя процедурами по `proc_call_graph`. BSL-вариант универсального `find_path` — `proc_call_graph` хранит `call_type` и ключи процедур. **(v0.35.0)** ключи процедур — `<rel_path>::<name>`; обход по резолвленному `callee_proc_key`. **(v0.36.0)** BSL хранит callee склеенно (`Модуль.Метод`, как `obj.method` у Python); цель вызова резолвится точно в адрес общего модуля (`…/CommonModules/X/Ext/Module.bsl::M`) и менеджер-модуля (`…/Catalogs/X/Ext/ManagerModule.bsl::M`), объектный шум отсеян — резолв direct-рёбер ~80-82%. **v1.0.0:** обход — поиск в ширину с учётом посещённых узлов вместо рекурсивного запроса (плотный граф отвечал 494 с, стало 112 мс), потолок узлов и бюджет времени с признаками `walk_nodes_capped`/`walk_stopped_by_time`, рядом `visited_nodes`; `from` принимается голым именем процедуры — сервер сам подберёт подходящие ключи |
| `get_role_rights` | **Права ролей одним вызовом (v1.0.0):** по объекту (`Catalog.Организации`) — роли со списком прав каждой; по роли (`ПолныеПрава`) — объекты с правами. Данные из таблицы `role_rights`, имена прав как в конфигурации (`Read`, `Insert`, `Update`, `View`…). Заменяет обход каталога `Roles` с чтением `Rights.xml` по кускам и произвольный SQL для типового вопроса; в ответе счётчики `roles_count`/`objects_count` и признак обрезки |
| `search_terms` | **Смысловой поиск процедур (v0.30.0):** термы наполняются механически при индексации — слова имени процедуры (CamelCase-сплит), имя и синоним объекта-владельца, комментарий над процедурой. **v1.0.1-ka.3:** метатеги `@tags` из описания процедуры и из шапки модуля — отдельной колонкой с бОльшим весом в ранжировании (тег ставит разработчик руками, это сигнал сильнее случайного совпадения слова в описании); описание больше не режется вместе с тегами на 240 символах, в ответе рядом с термами идут `tags`, `comment_head` (первая строка описания) и `module_tags`. LLM не нужен. Триграммный FTS: словоформы и подстроки от 3 символов, регистр и ё/е не важны; многословный запрос ищется как OR по словам (лучшие совпадения сверху). Первый выбор для «где реализован функционал X». LLM-обогащение (`bsl-indexer enrich`) остаётся опциональной надстройкой |
| `get_data_links` | **Граф связей данных (v0.10.0):** на что ссылается объект / кто ссылается на него — по ссылочным реквизитам, измерениям регистров и реквизитам табличных частей (таблица `data_links`). `direction=out\|in\|both`, `depth=1..4`. Заменяет серию `get_object_structure` при трассировке связей. Цели вида `*CatalogRef`/`*AnyRef`/`*DefinedType.X` — обобщённые ссылки (терминал, не разворачиваются). **v0.56.0:** обход — поиск в ширину с учётом посещённых узлов вместо рекурсивного запроса по путям (на центральном справочнике направление «кто ссылается» на глубине 4: 42,9 с → доли секунды), прерывание по времени 8 с с признаком `walk_stopped_by_time`, потолок числа рёбер. Рёбра отдаются страницей по размеру ответа (`offset`, `<dir>_shown`/`_total`/`_offset`/`_has_more`), рядом `<dir>_by_kind` — счётчики по видам связи, чтобы сузить запрос вместо перебора страниц. На глубине 1 полное число рёбер точное; глубже, если обход упёрся в потолок или время, оно помечается `<dir>_total_partial`. **Ссылки форм:** рёбра `form_attr` / `form_param` / `form_main_table` (тип реквизита или параметра формы, основная таблица динамического списка) от владельца формы, `from_path` вида `Form.<Имя>.<Реквизит>` |
...
Замеры на одной машине под Windows с обычным жёстким диском, август 2026.
...
Запуск на готовом индексе использует `mtime + file_size` fast-path: только
`stat()` на каждый файл, ни одного чтения содержимого, ни одного SHA-256.

| Метрика | Значение |
|---------|----------|
| Функций (Управление Торговлей) | 261 548 |
| Вызовов (граф) | 1 962 941 |
...
|----------|------|------------|
| Найти функцию по имени | O(n) файлов, секунды | миллисекунды |
| Кто вызывает функцию X? | grep по всем файлам | миллисекунды |
| Карта файла | cat + анализ | миллисекунды |
| Полнотекстовый поиск | grep -r, секунды | миллисекунды |
