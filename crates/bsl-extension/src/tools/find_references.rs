...
impl IndexTool for FindReferencesTool {
    ...
    fn description(&self) -> &str {
        "Всё, что ссылается на объект метаданных 1С, одним вызовом — «карта \
         ...
         типы, реквизиты и параметры форм, основная таблица динамического списка); \
         code_usages — обращения в коде и запросах (.bsl: менеджер коллекции, \
         тип-ссылка, путь в запросе; запросы схем компоновки в Template.xml и \
         динамических списков в Form.xml); role_rights — какие роли выдают права \
         на объект. У каждой \
