...
impl IndexTool for BslSqlTool {
    ...
    fn description(&self) -> &str {
        "Произвольный read-only SQL (SELECT/WITH) по таблицам BSL-индекса репо 1С. \
         СРАЗУ бери его на вопросы со словом СКОЛЬКО («сколько объектов каждого \
         вида», «сколько форм у документов», «у скольких объектов есть модуль») — \
         это один SELECT с GROUP BY по metadata_objects / metadata_forms / \
         metadata_modules вместо сотен вызовов list_files; и на вопросы про ПРАВА \
         РОЛЕЙ, если нужен срез сложнее, чем даёт get_role_rights (таблица \
         role_rights). Вообще любые «сколько/каких/у кого» с фильтром, join или \
         подсчётом — сюда. Перебор объектов, ролей, модулей или каталогов ради \
         счёта — верный признак, что вопрос надо было задать сюда одним запросом. \
