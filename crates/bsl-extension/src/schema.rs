...
pub const SCHEMA_EXTENSIONS: &[&str] = &[
    ...
    // ── exported_procs ────────────────────────────────────────────────────
    // Справочник экспортных процедур конфигурации: где какое имя объявлено
    // экспортным.
    //
    // Зачем нужен. Чтобы понять, куда ведёт вызов из изменившегося модуля,
    // мало самого модуля: в коде 1С вызов записан как `Модуль.Метод` или
    // просто `Метод`, и адрес выводится только знанием всей конфигурации —
    // в каком модуле это имя экспортно и не объявлено ли оно где-то ещё.
    // Раньше такой справочник собирался заново на КАЖДЫЙ пакет изменений,
    // полным перебором всех процедур с текстовым условием `args LIKE
    // '%) Экспорт%'` (экспортность нигде не хранилась). На типовой торговой
    // конфигурации это 9,4 секунды на пакет — независимо от того, изменился
    // один файл или тысяча.
    //
    // Теперь справочник постоянный: наполняется целиком при первичной
    // индексации и ведётся пофайлово (изменился модуль — переписаны строки
    // только этого пути).
    //
    // `kind` — вид модуля, от него зависит способ адресации вызова:
    //   * `common`  — общий модуль, вызов вида `ОбщегоНазначения.Метод`;
    //     `owner` = имя общего модуля;
    //   * `manager` — модуль менеджера, вызов `Справочники.Контрагенты.Метод`;
    //     `owner` = имя объекта, `folder` = папка типа (`Catalogs`);
    //   * `other`   — прочие модули; участвуют только в проверке
    //     «это имя где-нибудь экспортно».
    "
    CREATE TABLE IF NOT EXISTS exported_procs (
        repo TEXT NOT NULL,
        path TEXT NOT NULL,
        name TEXT NOT NULL,
        kind TEXT NOT NULL,
    ...
    ",
    "CREATE INDEX IF NOT EXISTS idx_ep_name ON exported_procs(repo, name);",
    "CREATE INDEX IF NOT EXISTS idx_ep_owner ON exported_procs(repo, kind, owner, name);",
    "CREATE INDEX IF NOT EXISTS idx_ep_folder ON exported_procs(repo, kind, folder, owner, name);",
    "CREATE INDEX IF NOT EXISTS idx_ep_path ON exported_procs(repo, path);",
    ...
    //     | `query` (путь метаданных в тексте запроса) | `dcs_query` (запрос
    //     набора данных СКД в Template.xml) | `form_query` (ручной запрос
    //     динамического списка в Form.xml). У двух последних `file_path` —
    //     путь XML, а не модуля.
