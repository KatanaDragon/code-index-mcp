...
///
/// Текст обязан вести к секции, а не отговаривать от неё: прежняя редакция
/// утверждала, что полный набор значений перечисления «дампом недоступен», и
/// отправляла искать значения по коду. После появления страниц по секции это
/// стало неправдой — выпущенная возможность оставалась невидимой для модели.
...
Любая опущенная секция доступна ПОЛНОСТЬЮ: запроси объект с узким sections=[<секция>] — \
одна секция помещается в ответ, а если она велика, придёт страницами (в ответе будут \
`<секция>_shown`/`<секция>_total` и готовый вызов следующей страницы с offset=). \
Нужно проверить одно конкретное значение — быстрее grep_code/grep_body по его имени.";
