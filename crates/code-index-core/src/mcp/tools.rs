...
(без скобок) — проверьте через get_function/find_symbol. Голое имя процедуры общего или \
менеджер-модуля 1С ищется и как `Модуль.Имя` / `Справочники.X.Имя`; чтобы спросить про процедуру \
ОДНОГО модуля, отсекая одноимённые, передайте квалифицированное имя `Модуль.Имя`. \
