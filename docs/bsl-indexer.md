...
```json
{
  "query": "весы настройки",
  "results": [
    {
      "proc_key": "CommonModules/КРБ_ИнтеграцияВесов/Ext/Module.bsl::ПрочитатьНастройки",
      "terms": "весы, настройки, прочитать настройки, читает настройки подключения…",
      "tags": "весы, настройки",
      "comment_head": "Читает настройки подключения к службе весов из безопасного хранилища БСП.",
      "module_tags": "весы, служба, настройки, интеграция",
      "signature": "mech:v2",
      "score": -8.2
    }
  ]
...
`tags` / `comment_head` / `module_tags` присутствуют, только когда у процедуры есть описание
или метатеги; у строк LLM-обогащения их нет.

#### Метатеги `@tags` и шапка модуля

Термы процедуры собираются механически при индексации из пяти источников — в этом порядке:
метатеги `@tags` описания процедуры, слова имени (CamelCase-сплит), имя объекта-владельца,
его синоним, проза описания (первые 240 символов) и метатеги `@tags` ШАПКИ модуля.

Метатег — строка `@<имя> <значение>` в комментарии (грамматика `code-docs`):
...
* значения `@tags` разделяются запятой, `@depends` — точкой с запятой;
* строки метатегов в прозу не попадают, поэтому `@security` и `@layer` термов не создают
  (шума от них больше, чем сигнала), а `@tags` в конце длинного описания не теряются:
  предел 240 символов действует только на прозу;
* колонка `procedure_enrichment.tags` вынесена в отдельную колонку FTS и ранжируется
  с весом 4 против 1 у прозы: тег ставит разработчик руками;
* `comment_len` — длина описания до обрезки. `0` означает «описания над объявлением нет»,
  по этой колонке считается аудит «экспортные процедуры без контракта»;
* шапка модуля — комментарий в начале `.bsl` до первого объявления (блок считается шапкой,
  если за ним идёт пустая строка, `#Область`, `#Если`, `Перем` или конец файла). Её проза
  и метатеги лежат в `module_enrichment(path, header, tags, depends)`, а теги дополнительно
  попадают в термы всех процедур модуля — поэтому отдельного поиска по модулям не нужно.
  Первую строку шапки и её теги показывает `get_object_profile` в секции `modules`.

Смена алгоритма сборки термов меняет подпись механических строк (`mech:v1` → `mech:v2`).
При открытии базы демон видит устаревшую подпись и пересобирает ТОЛЬКО слой термов —
по сохранённому в базе тексту модулей, без переиндексации метаданных и графов
(на конфигурации КА это десятки секунд против 394 с полной индексации). Пока проход идёт,
папка числится индексируемой и `health` отвечает «не готов». Запасной путь, если проход
почему-то не отработал, — обычная полная индексация `bsl-indexer index --config daemon.toml`.
