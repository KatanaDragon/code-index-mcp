...
| FTS по бизнес-терминам | `search_terms` (bsl-ext) | всегда: термы собираются механически при индексации, LLM-обогащение — опциональная надстройка поверх |
...
### Механические термы, метатеги и шапка модуля

Слой термов (`crates/bsl-extension/src/terms.rs`) собирается без LLM и живёт в двух таблицах:

* `procedure_enrichment(proc_key, terms, tags, comment_head, comment_len, signature)` —
  одна строка на процедуру. `terms` — фразы через запятую в порядке: метатеги `@tags`
  описания, слова имени (CamelCase-сплит), имя объекта-владельца, его синоним, проза
  описания (до 240 символов), метатеги `@tags` шапки модуля. `tags` — отдельная КОЛОНКА
  FTS: запрос без указания колонки ищет по обеим, а `bm25(fts, 1.0, 4.0)` взвешивает
  попадание в тег выше попадания в прозу. `comment_len` — длина описания до обрезки
  (`0` = описания нет), `comment_head` — его первая строка для выдачи агенту.
* `module_enrichment(path, header, tags, depends, signature)` — одна строка на `.bsl`
  с шапкой: комментарий в начале файла до первого объявления. Блок считается шапкой,
  если за ним идёт пустая строка, `#Область`, `#Если`, `Перем` или конец файла; блок,
  который без пустой строки переходит в `Процедура`/`Функция`/`&Аннотация`, — это
  описание первой процедуры, а не шапка. Теги шапки идут и в термы процедур модуля,
  поэтому отдельного поиска по модулям не нужно.

Грамматика метатегов общая с правилами разметки кода 1С (`code-docs`): строка
`@<имя> <значение>`, значения `@tags` через запятую, `@depends` — через точку с запятой.
Индексируются только `@tags`; `@layer`, `@security`, `@todo`, `@example` в термы не идут
(шума от них больше, чем сигнала), но и в прозу не попадают.

Три пути заполнения дают одинаковый результат: сборщик фазы парсинга (staging-таблицы
`_proc_terms_staging` / `_module_header_staging` → `build_procedure_terms_from_staging`),
полный disk-проход `index_procedure_terms` и пофайловый инкремент
`update_procedure_terms_for_file`. Строки LLM-обогащения (подпись не `mech:%`) ни один
из них не трогает.

Алгоритм сборки версионируется подписью `MECH_SIGNATURE` (сейчас `mech:v2`). При открытии
базы `migrate_extensions` видит строки со старой подписью и пересобирает ТОЛЬКО слой термов
по сохранённому в базе тексту модулей (`file_contents`) — без переиндексации метаданных и
графов; FTS-индекс при этом мигрирует на две колонки с `rebuild` и пересозданием триггеров.
