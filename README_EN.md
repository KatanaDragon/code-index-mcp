...
[Русская витрина](README.md) · [Full Russian manual](README_RU.md)
...
- **Performance.** An 88K-file accounting configuration indexes from scratch in 5 min 51 s, starts up on an existing index in 5.3 s, and answers a query in about 10 ms. Production-grade for 100K+ file monorepos.
- **32 MCP tools.** 20 universal (functions, classes, callers/callees, file content, grep) + 12 BSL-tools (object structure & profile, form handlers, event subscriptions, call graph, data links, register writers, role rights, impact map, read-only SQL).
- **Native BSL/1C.** Parses both Configurator (XML) and 1C:EDT (`.mdo`) exports of 1C:Enterprise 8.3 configurations. Data-link graph (object→object edges via reference types in attributes) — 65,421 edges for a typical accounting configuration.
...
| `get_callers` | Who calls this function? A bare name of a 1C common- or manager-module procedure is expanded with `Module.Name` / `Справочники.X.Name` forms derived from its definition site (contour build 2026-09-08); a qualified name is looked up as is — that is how to ask about one module's procedure. **(v0.35.0)** each row carries the caller's source `path` (distinguishes same-named callers from different files). **v0.62.0:** for 1C configurations, form event handler bindings (`kind: form_binding`) are added to the code callers — such a procedure is run by the platform per an entry in the form description and has no call in the code; when the name belongs to several procedures, a single record with a sample `get_form_handlers` call arrives instead of a list |
| `get_callees` | What does this function call? **(v0.35.0)** each row carries the source `path` |
| `find_path` | **(v0.23.0)** Shortest path in the call graph between two functions `from`→`to` (iterative cycle-safe BFS over unique `calls` nodes, `max_depth=5`, any language). Returns path edges `[{caller, callee, line}]`. **v0.57.0:** an empty answer carries walk cut-off flags `walk_depth_exhausted` / `walk_nodes_capped`, and the hint distinguishes «hit the node cap», «ran out of depth» and «exhausted the reachable subgraph» — all three used to look like «no path» |
...
| `grep_code` | **(v0.8.0)** Regex search over **code-file** content (`.py`, `.bsl`, `.rs`, `.ts`, etc.) via `file_contents` table (zstd-decode in Rust). Same parameters as `grep_text`: `regex`, `path_glob?`, `language?`, `limit?`, `context_lines?`. Since v0.66.0 both tools also accept `pattern` — a literal, case-insensitive substring (escaped before the search), the same meaning it has in `grep_body`. Complements `grep_body` (which searches only inside function/class bodies). Oversize files are skipped. **`scope` parameter** (`code` / `text` / `all`) on both tools: 1C object XML files (`Catalogs/X.xml`, documents, registers, subsystems) are stored as code with `language=xml_1c`, while `Configuration.xml`, `Form.xml`, `Rights.xml` are stored as text, so "where does this name occur in the dump" used to take two calls with different filters; `scope="all"` searches both categories in one call (code first, text on the remaining shared `limit`) and reports `by_category` and `truncated_by_category`; `language` narrows code only. Without `scope` behaviour is unchanged. Remote repos on builds without `scope` are served by two forwards merged on the caller side |
| `health` | MCP server health and connected repos |

All search tools (`search_function`, `search_class`, `get_function`, `get_class`, `find_symbol`, `search_text`, `grep_body`) accept an optional **`path_glob`** parameter (v0.7.0) to scope results to a subtree (e.g., `src/auth/**`, `Documents/**/*.bsl`). Implementation: post-filter via the `globset` crate after the SQL query. Since v0.32.0, `path_glob`/`pattern` support `{a,b}` brace alternates (`**/*.{bsl,xml}`) — including in `grep_code`/`grep_text`/`grep_body`/`list_files`, where the filter runs at the SQLite GLOB level (the pattern is expanded into an OR group of conditions). In `search_function`/`search_class`/`search_text` the post-filter runs after records are fetched by relevance, so a scoped result may be incomplete; since v0.49.8 such a response carries `prefilter_exhausted` together with `prefetched`/`shown` — on non-empty results too, not only on empty ones.
...
| Tool | Description |
|------|-------------|
| `get_object_structure` | Full structure of a 1C metadata object by `full_name` (`Document.SalesInvoice`): attributes with 1C-notation types (+`synonym` — UI label, +`required` — fill checking, v0.32.0; +`indexing` — indexing flag `Index`/`IndexWithAdditionalOrder`, v0.45.1), tabular sections with columns, register dimensions/resources; `enum_values` for enumerations (+`enum_synonyms` — UI labels of values); `predefined` for objects with predefined items (catalogs, charts of accounts); `posting` for documents (posting properties from root `<Properties>`); `owners` — owners of a subordinate catalog; `value_types` — value type of a CCT/constant (for a CCT — available analytics); `properties` — header properties (register periodicity, write mode, numbering, hierarchy); `commands` — object commands with synonyms (all — v0.32.0). Base sections (`attributes`/`dimensions`/`resources`/`tabular_sections`) are always present (empty as `[]`). `sections` parameter (v0.29.0) — narrow selection: return only the requested sections (e.g. `["posting"]` — ~0.2 KB instead of the full object). **Criterion selector (v0.41.0):** `name_like` (object-name substring) + optional `meta_type` return the structures of ALL matching objects in one call (`{matched, truncated, results}`, cap 50) — for a thematic set (e.g. `name_like="EDI"`) instead of calling one by one. **v0.49.0:** `names_only=true` — only each object's passport (`full_name`/`meta_type`/`name`/`synonym`) without the structure, the cheapest search mode; `max_response_bytes` — a size budget for this response on top of the server-wide one, when the full sections are needed. **v0.55.0:** `offset` — pages over ONE requested section (`sections=['enum_values']`): the page is filled by response size, with `<section>_shown`/`_total`/`_offset`/`_has_more` alongside and a ready-to-copy call for the next page. Before that a section of hundreds of values was dropped by the guard entirely and was unreachable |
| `get_form_handlers` | Managed-form event handlers by `(owner_full_name, form_name)`. Owner accepted in both formats — `Document.X` and export-folder `Documents.X` (v0.31.0). **v0.54.0:** returns `(event, handler, element)` triples — `element` is the form item the handler belongs to (absent for form-level handlers); an optional `element` filter narrows the response to one item, an empty string — form-level handlers only (a large form: 308 records → 8), and `handlers_shown`/`handlers_total` are added. Event names are normalized to Russian in both dump formats — a dictionary of 91 events. Unknown form → error with `available_forms` of the owner. **v0.55.0:** `form_name` is optional — without it, handlers of ALL forms of the object in one call (it used to be 255 calls for an object with 255 forms); a `max_response_bytes` budget was added. When it does not fit — a listing of forms with handler counts, or per-item counts within one form, and in both cases a ready-to-copy call for the piece you need |
...
| `search_terms` | **Meaning-based procedure search (v0.30.0):** terms are filled mechanically at index time — words of the procedure name (CamelCase split), the owner object's name and synonym, the comment above the procedure. **v1.0.1-ka.3:** `@tags` metatags of the procedure's description and of the module header — a separate column with a higher ranking weight (a tag is written by hand, a stronger signal than a word that happens to occur in prose); the description is no longer truncated together with the tags at 240 characters, and the response carries `tags`, `comment_head` (first line of the description) and `module_tags` next to the terms. No LLM needed. Trigram FTS: word forms and 3+ character substrings work, case and ё/е are irrelevant; a multi-word query is searched as an OR of words (best matches first). The first choice for "where is feature X implemented". LLM enrichment (`bsl-indexer enrich`) remains an optional add-on |
| `get_data_links` | **Data-links graph (v0.10.0):** what an object references / what references it, via reference-typed attributes, register dimensions and tabular-section attributes (`data_links` table). `direction=out\|in\|both`, `depth=1..4`. Replaces a series of `get_object_structure` calls when tracing relations. Targets like `*CatalogRef`/`*AnyRef`/`*DefinedType.X` are generalized refs (terminal, not expanded). **v0.56.0:** the walk is a breadth-first search with a visited set instead of a recursive path-based query (on a central catalog, «what references it» at depth 4: 42.9 s → a fraction of a second), an 8-second time cut-off with a `walk_stopped_by_time` flag, and an edge cap. Edges come as a page sized by the response budget (`offset`, `<dir>_shown`/`_total`/`_offset`/`_has_more`), with `<dir>_by_kind` counts alongside so you can narrow the query instead of paging through everything. At depth 1 the total edge count is exact; deeper, if the walk hit the cap or the deadline, it is marked `<dir>_total_partial`. **Form references:** edges `form_attr` / `form_param` / `form_main_table` (type of a form attribute or parameter, main table of a dynamic list) from the form owner, `from_path` shaped `Form.<Name>.<Attribute>` |
| `find_data_path` | **Data-links graph (v0.10.0):** chain of reference links from one object to another (BFS over `data_links`, like `find_path` but for data, not calls). **v0.57.0:** on failure the answer distinguishes «no path» from «not enough hops» — flags `depth_exhausted`, `visited_nodes` and a ready call with a larger depth |
...
| `get_object_profile` | **Object passport in one call (v0.21.0):** the full portrait of an object — structure + forms + modules + data links — instead of a series of `get_object_structure`/`get_form_handlers`/`get_data_links`. The `sections` parameter (`['structure'\|'forms'\|'modules'\|'data_links']`) narrows the response. **v0.55.0: an overview instead of a dump** — forms come as a listing with handler counts, modules as counts by type, data links as counts by kind only (no edges: their volume is set by the whole configuration, not by the object). Content is fetched via `expand` (`forms.handlers`, `modules.list`) or arrives on its own when the response fits the budget (flags `forms_handlers_included`, `modules_listed`); every folded section carries a ready-to-copy call for its content. On an object with 255 forms the response went 336 KB → 19 KB. **v1.0.1-ka.3:** modules that have a header (the comment at the top of the `.bsl`) carry its first line (`header`) and the `@tags` metatags; the folded module map carries the merged tag list |
| `find_references` | **Impact map (v0.21.0):** everything that references an object, in one call — reverse `data_links` (structural refs from metadata) + `metadata_code_usages` (usages in `.bsl` code) + `role_rights` (roles holding rights on it), broken down by kind with samples (`limit`). **v0.55.0:** `section` (`data_refs`/`code_usages`/`role_rights`) + `offset` — one section in full, paged by response size, with a ready-to-copy call for the next page: on a central catalog 2,443 code usages are collected in 10 turns, whereas nothing could take you past 200 samples before. **Sources extended:** `data_refs` includes references from form descriptions (attributes, parameters, main table of a dynamic list), `code_usages` — queries of data composition schemas (`dcs_query`, `Template.xml`) and dynamic lists (`form_query`, `Form.xml`) with the XML file path and line |
| `get_role_rights` | **Role rights in one call (v1.0.0):** given an object (`Catalog.Организации`) — the roles holding rights on it, each with its right names; given a role (`ПолныеПрава`) — the objects it may act on. Served from the `role_rights` table, right names as spelled in the configuration (`Read`, `Insert`, `Update`, `View`…). Replaces walking the `Roles` directory reading `Rights.xml` piece by piece, and raw SQL for the common case; the response carries `roles_count`/`objects_count` and a truncation flag |
| `bsl_sql` | **Arbitrary read-only SQL (v0.21.0):** a `SELECT`/`WITH` query over the repo's `index.db` for the long tail of metadata/graph questions that have no dedicated tool (roles/RLS, joins, aggregations). Guard: `SELECT`/`WITH` only + `Statement::readonly()` + row cap + timeout. Tables: `metadata_objects`, `metadata_modules`, `metadata_forms`, `event_subscriptions`, `data_links`, `role_rights`, `proc_call_graph`, `procedure_enrichment` (terms, `tags`, `comment_head`, `comment_len`), `module_enrichment` (module header: `header`, `tags`, `depends`), core `functions`/`files`. An empty result over the procedure tables auto-falls back to `search_terms` |

**Object name case no longer matters (v0.57.0).** 1C object names are Cyrillic and SQLite does not fold their case, so `Catalog.организации` used to return a silent zero: data links answered «nothing references this object», recorders — «no recorders». The name is now resolved to the spelling stored in the configuration at the entry of the object-keyed tools (`get_data_links`, `find_data_path`, `get_register_writers`, `get_object_structure`, `get_object_profile`, `find_references`) and echoed back canonically. Did-you-mean matching stopped depending on case as well. The schema gained a key column and an index; **no reindex is required** — the migration fills the keys on the first daemon start.
...
max_response_bytes_hard = 192000  # ceiling for a client-supplied max_response_bytes; 0/absent — 4x the budget (v0.49.0)
...
- **`shrink_search_results`** (v0.49.0, for a SEARCH response of `get_object_structure` via `name_like`/`full_names`) — identification of what was found is untouchable. Heavy sections INSIDE the items are dropped first (`<section>_omitted` + `<section>_count`); only if that is not enough is the array itself shortened, with `results_total`/`results_shown`. The array is never emptied at any budget above zero, and `full_name`/`meta_type`/`name`/`synonym` stay on every item. Previously the heaviest section was the `results` array itself, so it went first — the client got `{matched: 38, results_omitted: true}` without a single name.
- **`max_response_bytes` as a call parameter** (v0.49.0, `get_object_structure`) — a budget for one response on top of the server-wide one. A request above `max_response_bytes_hard` is clamped to the ceiling, and the applied value comes back in `response_budget_applied`. Zero (disable the guard) is not allowed per call — only via server config.
...
Measured on a single Windows machine with a spinning disk, August 2026.

| Codebase | Files | Full index | Startup on an existing index |
|---|---:|---:|---:|
| 1C:Trade Management | 57,072 | 2 min 41 s | **2.3 s** |
| 1C:Accounting | 88,284 | 5 min 51 s | **5.3 s** |
| PHP website | 157,772 | 13 min 1 s | **8.0 s** |

A full index is a from-scratch parse, done once. For Accounting those 5 min 51 s
break down into 2 min 38 s core (syntax trees and writes), 1 min 58 s of the 1C
layer (metadata, forms, rights, call graph) and 56 s flushing the database to disk.

Startup on an existing index uses the `mtime + file_size` fast-path: only `stat()`
per file, zero content reads, zero SHA-256 hashes.

| Metric | Value |
|--------|-------|
| Functions indexed (Trade Management) | 261,548 |
| Call graph edges | 1,962,941 |
| Query response | about 10 ms over HTTP, sub-millisecond when cached |
| Binary size | 39 MB without 1C, 41 MB with 1C |

Comparison with grep:

| Operation | grep | Code Index |
|-----------|------|------------|
| Find function by name | O(n) files, seconds | milliseconds |
| Who calls function X? | grep all files | milliseconds |
| File map | cat + manual analysis | milliseconds |
| Full-text search | `grep -r`, seconds | milliseconds |
...
**Session-level re-delivery filter (0.48.0).** Rows already delivered within the same session are not sent again: whatever is dropped is marked with `rows_elided_already_delivered` plus a hint, so a shortened or empty list never silently reads as "no data" — a result that was empty to begin with carries no marker, which keeps the two cases apart. The filter scope is "tool + repository", so identical rows from different bases never cancel each other out. Turn it off with `dedup_enabled = false` under `[mcp]` in `daemon.toml`; clear its memory with `POST /dedup-reset` — useful when the client has compacted its context, an event not observable from inside MCP.
